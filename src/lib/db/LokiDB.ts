...
//   if (Utils.isntNull(db) && initialized) {
//     log.info('db saving');
...
//  */
// export function init(adapterType = null): Promise<any> {
...
//     LOC = process.env.YK_DB_STORAGE_DIR || 'db';
//     DBPATH = path.join(LOC, 'yt4kids.db');
...
//         log.info('created directory', LOC);
//       }
//     } else {
...
//  */
// function ensureDatabaseIsOpen(performInit = false) {
//   if (Utils.isNull(db) || initialized === false) {
//     if (performInit) {
//       return init();
//     }
...
// async function getOrCreateDB(type: RecordTypes) {
...
// export async function findOneType<T extends YTRecord>(type: RecordTypes, id: string): Promise<T | undefined> {
//   const val = await findOne(type, id);
//   return val as T;
// }

// /**
//  * finds a single record by key
//  * @param {RecordTypes} type what are we retrieving
//  * @param {string} id key to return
//  * @returns a single record
//  */
// export async function findOne(type: RecordTypes, id: string) {
//   // ? obsolete with TS?
//   if (Utils.isNull(id)) {
...
//   // just use the type as a collection
//   let coll = await getOrCreateDB(type);
...
//  */
// async function fetch(type: RecordTypes, options: QueryOptions): Promise<Resultset<any>> {
//   if (Utils.isNull(options) || Object.keys(options).length === 0) {
//     log.info('missing options for find', options);
...
//   if (options?.clause !== undefined) {
//     results = results.find(options.clause);
//   }
//   if (options?.sortBy !== undefined) {
//     results = results.simplesort(options.sortBy);
//   }
...
//  * @param {QueryOptions} [options] object with query clauses
//  */
// export async function find(type: RecordTypes, options: QueryOptions) {
//   log.info('find', type, options);
//   const results = await fetch(type, options);
//   return results.data();
// }
...
//  * @param {QueryOptions} [options] object with query clauses
//  */
// export async function removeAll(type: RecordTypes, options: QueryOptions) {
//   log.info('removeAll', type, options);
//   const results = await fetch(type, options);
...
//  */
// export async function remove(type: RecordTypes, id: string) {
//   log.info('remove called', type, id);
...
//  */
// export async function insertOrUpdateObj<T extends YTRecord>(obj: T) {
//   await ensureDatabaseIsOpen();
//   if (!(obj.recordType in RecordTypes)) {
//     log.info('unknown type', obj.recordType);
//     throw new Error(`unknown type: ${obj.recordType}`);
//   }

//   if (Utils.isNull(obj.id)) {
//     log.info('missing id for type', obj.recordType);
//     throw new Error(`object of type ${obj.recordType} has no id`);
//   }

//   // just use the type as a collection
//   let coll = await getOrCreateDB(obj.recordType);
//   // poor mans clone
